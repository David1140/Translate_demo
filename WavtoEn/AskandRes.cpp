...
std::string client_id = "";//key_id 需要输入API_KEY
std::string client_secret = "";//填入API_SECRET
...
int main(int argc, char *argv[])
{
    ...
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "POST");
        curl_easy_setopt(curl, CURLOPT_URL, ("https://aip.baidubce.com/rpc/2.0/mt/v2/speech-translation?access_token=" + getAccessToken()).c_str());
        curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
        curl_easy_setopt(curl, CURLOPT_DEFAULT_PROTOCOL, "https");
        struct curl_slist *headers = NULL;
        ...
        headers = curl_slist_append(headers, "Accept: application/json");
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
        const char *data = ("{\'from\':\'zh\',\'to\':\'en\',\'format\':\'wav\',\'voice\':\'//" + getFileBase64Content("./voice_temp/temp.wav") +"\'}'").c_str();
