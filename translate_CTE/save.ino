...
//设置gpio口
#define key 0//ESP8266:0 / ESP32:15
#define ADC A0 //ESP32:39 /ESP9266:A0
//#define LED_BUILTIN LED_BUILTIN//ESP8266 LED_BUILTIN ESP32:2     //SCL=5,SDA=4
#define speaker 4
...
String ch_to_en(const char*name);                       //中文语音to英文文本
...
String ch_to_en(const char* filename)    
{
    ...
    if (File1) {
      ...
      client.print(String(test.length()+fileSize+2));
      client.print("\r\n\r\n");
      client.print(test);
      // 从文件中读取内容
...
{
  // 打开输入文件
  File inFile = SD.open(inputFile,"r");
  if (!inFile) {
  ...
  while (inFile.available()) {
    size_t bytesRead = inFile.read(buffer, 500);
    // 将编码后的内容写入输出文件
    outFile.print(base64::encode(buffer,sizeof(buffer)));//.c_str()
  }
